...
public class SpatialClusterer {
  private static double prune = 0.0d;
  private static double threshold = 0.01d;
  ...
  public static Map<Integer,Collection<Integer>> cluster(
    ...
    m = inflate(m, 3.0d);
    //m = sharpen(m);
    //System.out.println("pre inflation " + m.toString());
    int iterations = 0;
    SimpleMatrix totalM = m.copy();
    do {
      ...
      m = totalM;
      m = prune(m, prune);
      for (int s = 0; s < sharpens; s++) {
        m = sharpen(m);
        //System.out.println(s + " sharpen computed " + m.toString());
      }
